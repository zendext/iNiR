...
Singleton {
    ...
    function leapMonth(year: int): int {
        return lunarInfo[year - minYear] & 0x0f
    }

    function leapDays(year: int): int {
        if (leapMonth(year)) return (lunarInfo[year - minYear] & 0x10000) ? 30 : 29
        return 0
    }
    ...
    function lunarMonthDays(year: int, month: int, isLeap: bool): int {
        if (isLeap) return leapDays(year)
        return monthDays(year, month)
    }

    function lunarYearDays(year: int): int {
        let sum = 348
    ...
    function toLunar(date: var): var {
        if (!date) return null
        const year = date.getFullYear()
        if (year < minYear || year > maxYear) return null

        const baseUtc = Date.UTC(minYear, 0, 31)
        const targetUtc = Date.UTC(year, date.getMonth(), date.getDate())
        const offsetDays = Math.floor((targetUtc - baseUtc) / 86400000)
        if (offsetDays < 0) return null

        let offset = offsetDays
        let lunarYear = minYear
        let yearDays = lunarYearDays(lunarYear)
        while (lunarYear <= maxYear && offset >= yearDays) {
            offset -= yearDays
            lunarYear++
            yearDays = lunarYearDays(lunarYear)
        }
        if (lunarYear > maxYear) return null
        ...
        while (lunarMonth <= 12) {
            monthDaysCount = isLeap ? leapDays(lunarYear) : monthDays(lunarYear, lunarMonth)
            if (offset < monthDaysCount) break
